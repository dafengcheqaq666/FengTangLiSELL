...
export function ProductSelector({ product }: { product: StorefrontProduct }) {
  ...
  function addToCart() {
    add({ variantId: variant.id, productName: product.name, variantName: variant.name, priceFen: variant.priceFen, image: product.image, quantity });
    setAdded(true);
  ...
  return (
    <div className="product-panel">
      ...
      <h2>{product.name}</h2>
      <p className="product-sub">{product.subtitle}<br />收到后常温回甜 1–2 天，风味更佳。</p>
